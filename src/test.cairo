...
func main{output_ptr: felt*, range_check_ptr}() {
    ...
    // use uint256
    let (y3: Uint256) = sqrt_uint256(STEPS, Uint256(16,0));
    assert y3.low = 4;
    ...
    // sqrt using WAD high precision - sqrt(4*WAD) = 2*WAD
    let (y4: Uint256) = sqrt_hp(STEPS, Uint256(WAD+WAD+WAD+WAD,0));
    //assert y4.low = WAD + WAD;
